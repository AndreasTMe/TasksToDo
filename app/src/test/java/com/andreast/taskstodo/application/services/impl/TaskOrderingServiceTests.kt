...
class TaskOrderingServiceTests {
    ...
    @Test
    fun canReorderWhenMovingDown_tryingToPlaceFromItemBetweenItsChildren_returnFalse() {
        // Arrange
        val from = 0
        val to = 1
        val items = listOf(
    ...
    @Test
    fun canReorderWhenMovingDown_tooManyChildrenLevels_returnFalse() {
        // Arrange
        val from = 0
        val to = 7
        val items = listOf(
    ...
    @Test
    fun canReorderWhenMovingUp_tooManyChildrenLevels_returnFalse() {
        // Arrange
        val from = 6
        val to = 5
        val items = listOf(
            ...
            TaskListItemDto(id = 2, parentId = 1, order = 0, level = Level.Two),
            TaskListItemDto(id = 3, parentId = 2, order = 0, level = Level.Three),
            TaskListItemDto(id = 4, parentId = 3, order = 0, level = Level.Four), // to
            TaskListItemDto(id = 5, parentId = 3, order = 0, level = Level.Four),
            TaskListItemDto(id = 6, order = 1, level = Level.Zero), // from
            TaskListItemDto(id = 7, parentId = 5, order = 0, level = Level.One),
            TaskListItemDto(id = 8, parentId = 6, order = 0, level = Level.Two),
        )
        val shouldBecomeChild = false

        // Act
        val result = sut.canReorderWhenMovingUp(from, to, items, shouldBecomeChild)
    ...
    @Test
    fun reorderTasks_moveTaskWithSameAncestorUpAndBetweenSameLevelTasks_returnReorderedTasks() {
        // Arrange
        val from = 3
        val to = 2
        val items = listOf(
            ...
            TaskListItemDto(id = 1, parentId = 0, order = 0, level = Level.One), // to
            TaskListItemDto(id = 2, parentId = 0, order = 1, level = Level.One),
            TaskListItemDto(id = 3, parentId = 0, order = 2, level = Level.One), // from
            TaskListItemDto(id = 4, parentId = 0, order = 3, level = Level.One),
        )
        ...
        assert(result.isNotEmpty())
        assert(result.size == 4)
        assert(result[0].id == items[1].id && result[0].order == 0)
        assert(result[1].id == items[3].id && result[1].order == 1)
    ...
    @Test
    fun reorderTasks_moveTaskWithDifferentAncestorUpAndBetweenSameLevelTasks_returnReorderedTasks() {
        // Arrange
        val from = 5
        val to = 3
        val items = listOf(
            TaskListItemDto(id = 0, order = 0, level = Level.Zero),
            TaskListItemDto(id = 1, parentId = 0, order = 0, level = Level.One),
            TaskListItemDto(id = 2, parentId = 0, order = 1, level = Level.One), // to
            TaskListItemDto(id = 3, parentId = 0, order = 2, level = Level.One),
            TaskListItemDto(id = 4, parentId = 0, order = 3, level = Level.One),
            TaskListItemDto(id = 5, order = 1, level = Level.Zero), // from
        )
        ...
        assert(result.size == 5)
        assert(result[0].id == items[1].id && result[0].order == 0)
        assert(result[1].id == items[2].id && result[1].order == 1)
        assert(result[2].id == items[5].id && result[2].order == 2)
        assert(result[3].id == items[3].id && result[3].order == 3)
        assert(result[4].id == items[4].id && result[4].order == 4)
    }

    @Test
    fun reorderTasks_moveTaskUpAndBetweenLevelZeroAndOne_returnReorderedTasks() {
        // Arrange
        val from = 3
        val to = 1
        val items = listOf(
            TaskListItemDto(id = 0, order = 0, level = Level.Zero), // to
            TaskListItemDto(id = 1, parentId = 1, order = 0, level = Level.One),
            TaskListItemDto(id = 2, parentId = 1, order = 1, level = Level.One),
            TaskListItemDto(id = 3, order = 1, level = Level.Zero), // from
            TaskListItemDto(id = 4, order = 2, level = Level.Zero)
        )
        ...
        assert(result.any { it.id == items[2].id && it.order == 2 })
        assert(result.any { it.id == items[3].id && it.order == 0 })
    }

    @Test
    fun reorderTasks_moveChildTaskUpAndBetweenLevelZeroAndOne_returnReorderedTasks() {
        // Arrange
        val from = 3
        val to = 1
        val items = listOf(
        ...
        assert(result.isNotEmpty())
        assert(result.size == 4)
        assert(result[0].id == items[3].id && result[0].order == 0)
